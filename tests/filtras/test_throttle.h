...
TEST_CASE("ThrottleFT", "[throttle_filtra]"){
    ...
    SECTION("Correct rate"){
        MessageWrapper msg_w_1(std::make_shared<Message>(msg));
        throttle_ft.process_message(msg_w_1);
        REQUIRE(msg_w_1.is_passed());

        std::this_thread::sleep_for(std::chrono::seconds(1));

        MessageWrapper msg_w_2(std::make_shared<Message>(msg));
    ...
    SECTION("Burst of messages"){
        ...
        for(int i = 0; i < 5; ++i){
            MessageWrapper msg_w_burst(std::make_shared<Message>(msg));
